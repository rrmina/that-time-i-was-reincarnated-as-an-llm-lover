...
class CausalScaledDotProductAttention(nn.Module):
    def __init__(self, d_in, d_out, dropout=0.0):
        super(CausalScaledDotProductAttention, self).__init__()
    ...
    def forward(self, x):
        ...
        mask = torch.tril(torch.ones(seq_len, seq_len)).to(attn_scores.device)  # (seq_len, seq_len)
        attn_scores = attn_scores.masked_fill(mask == 0, float('-inf'))         # (batch_size, seq_len, seq_len)
